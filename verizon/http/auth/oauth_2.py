...
class Oauth2(HeaderAuth):
    ...
    def __init__(self, oauth_2_credentials, config):
        auth_params = {}
        self._oauth_client_id = oauth_2_credentials.oauth_client_id \
            if oauth_2_credentials is not None else None
        self._oauth_client_secret = oauth_2_credentials.oauth_client_secret \
            if oauth_2_credentials is not None else None
        if oauth_2_credentials is not None \
                and isinstance(oauth_2_credentials.oauth_token, OauthToken):
            self._oauth_token = OauthToken.from_dictionary(
                APIHelper.to_dictionary(oauth_2_credentials.oauth_token))
        else:
            self._oauth_token = oauth_2_credentials.oauth_token \
                if oauth_2_credentials is not None else None
        if oauth_2_credentials is not None \
                and isinstance(oauth_2_credentials.oauth_scopes, list):
            self._oauth_scopes = oauth_2_credentials.oauth_scopes
        else:
